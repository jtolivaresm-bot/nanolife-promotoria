/**
 * TEMPORAL — lee la pestaña Tiendas del sheet de retail intelligence para obtener las
 * direcciones oficiales de las salas. Borrar una vez extraídos los datos.
 * GET /.netlify/functions/debug-tiendas
 */

const SHEET_ID = "1a6ubAF5sZjVrqmtesLbgpi8mOl-6hIs2xkkMlbik5lY";
...
export const handler = async () => {
  ...
  try {
    ...
    const r = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent("Tiendas!A:Z")}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (!r.ok) throw new Error(`${r.status} ${await r.text()}`);
    const rows = (await r.json()).values || [];
    return { statusCode: 200, headers, body: JSON.stringify({ filas: rows.length, rows }, null, 2) };
  } catch (err) {
